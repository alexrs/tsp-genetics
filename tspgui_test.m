%Change in lines 49-75, and 106

function tspgui_test(CROSSOVER, NIND, MAXGEN,ELITIST,PR_CROSS,PR_MUT,LOCALLOOP, testName)
...
% load the data sets from the benchmark list
benchmarks = dir('TSPBenchmark/');
datasets=cell( size(benchmarks,1)-2,1);
for i=1:size(datasets,1)
    datasets{i} = benchmarks(i+2).name;
end

% start with first dataset
data = load(['TSPBenchmark/bcl380.tsp']);
